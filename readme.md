...
│   ├── llm.py            # 모델 설정 (GPT-4o)
│   ├── vector_store.py   # 청킹(Chunking) 및 MMR 기반 검색 관리 
│   ├── tools.py          # 검색 도구 연결
...
(2) 고도화된 RAG 검색 전략 (Vector Search Strategy) 
 단순한 데이터 검색을 넘어, 정보의 정확성과 다양성을 확보하기 위해 기술적 최적화를 수행했습니다.

최적화된 청킹(Chunking): RecursiveCharacterTextSplitter를 사용하여 문맥 손실을 최소화 (chunk_size=500).

MMR 검색 알고리즘: 단순 유사도 검색의 한계를 극복하기 위해 MMR(Maximal Marginal Relevance) 기법을 적용하여, 답변의 중복을 줄이고 정보의 다양성을 확보했습니다.
...
(6) 시스템 모니터링 (LangSmith) 
Traceability: 사용자의 질문부터 최종 답변까지 이어지는 전체 체인(Chain)을 추적합니다.
Latency Check: 각 단계별 소요 시간을 모니터링하여 병목 현상을 파악합니다.
